...
const Formcotacao = ({ onAddCotacao, initialValues }) => {
  const [produto, setProduto] = useState({ nome: '', preco: '', cnpj: '' });
  ...
  const handleAddProduto = () => {
    if (produto.nome && produto.preco && produto.cnpj) {
      const newProduto = { ...produto, preco: parseFloat(produto.preco) };
      setProdutos((prev) => [...prev, newProduto]);
      setProduto({ nome: '', preco: '', cnpj: '' });
    } else {
  ...
  const calcularTotal = (produtos) => {
    return produtos.reduce((acc, prod) => acc + prod.preco, 0);
  };
